...
export async function rpc(env,name,args,surface='creative'){
 const res=await fetch(surface==='raw'?'https://agent.livepeer.org/api/mcp/raw':endpoint,{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json, text/event-stream','Authorization':'Bearer '+bearer(env)},body:JSON.stringify({jsonrpc:'2.0',id:crypto.randomUUID(),method:'tools/call',params:{name,arguments:args}}),signal:AbortSignal.timeout(90000)});
 if(!res.ok)throw Error(res.status===401||res.status===403?'Livepeer rejected the saved PymtHouse credential.':`Livepeer returned HTTP ${res.status}. Please try again later.`);
...
export default {async fetch(request,env){
 ...
 if(!env.DAYDREAM_API_KEY?.trim())return json({error:'Save the PymtHouse key in DAYDREAM_API_KEY in site settings.'},503);
 try{
  ...
  switch(url.pathname){
   ...
   case '/api/livepeer/status':result=await rpc(env,'list_capabilities',{limit:1});return json({connected:true,mode:'authenticated',total:result.data.total??null});
